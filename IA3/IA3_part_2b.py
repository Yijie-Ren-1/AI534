...
def batch_kernalized_perceptron(X, Y, iter_num, X_val, Y_val, p):  
  '''
  ...
  while iter_count < iter_num:
    ...
    # u = np.dot(np.multiply(alpha, Y), kernalized_x_train)
    # alpha = np.where(np.multiply(u, Y) <= 0, alpha+1, alpha) 
    for i in range(len(X)):
      u = np.dot(np.multiply(alpha, kernalized_x_train[i]), Y)
      if u * Y[i] <= 0:
        alpha_temp[i] = alpha_temp[i] + 1
...
def batch_kernalized_perceptron_runtime(X, iter_num, Y):
  '''
  :param X: np 2d array, N*d, the training data
  :param iter_num: scalar, the maximum training iteration number
  :param Y: np 1d array, 1*N, the training label
...
def plot_acc_train_val(df_acc, acc_plot_save_path, p):
  ...
  acc.set_title('accuracy of train & val for batch kernalized perceptron, p = ' + str(p))
...
if __name__ == '__main__':
  ...
  print("p = 1")
  # Model training
  alpha, acc_train_list, acc_val_list = batch_kernalized_perceptron(X_train, Y_train, iter_num, X_val, Y_val, 1)

  acc_plot_save_path = "./plots/batch_kernelized_perceptron_acc_train_val_p_1.jpg"

  dict_acc = {
    'acc_train_p=1': acc_train_list,
    'acc_val_p=1': acc_val_list
  }

  df_acc = pd.DataFrame(dict_acc)
  plot_acc_train_val(df_acc, acc_plot_save_path, 1)


  # # draw runtime plot for different training size
  # runtime_plot_save_path = "./plots/batch_runtime_vs_training_size.jpg"
  # X_train_val = np.concatenate((X_train, X_val), axis=0)
  # Y_train_val = np.concatenate((Y_train, Y_val), axis=0)
  # runtime_list = []

  # for power in range(1,5):
  #   runtime = batch_kernalized_perceptron_runtime(X_train_val[:10 ** power,:], iter_num, Y_train_val[:10 ** power])
